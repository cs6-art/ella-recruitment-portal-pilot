...
export default function ApplicantEditForm({ applicant }: { applicant: ApplicantEditValues }) {
  ...
  const initialCountry = countryOptions.find((country) => country.country === applicant.applicantCountry.trim().toUpperCase()) || countryForPhone(applicant.contactNumber);
  const [country, setCountry] = useState(initialCountry.country);
  const [localNumber, setLocalNumber] = useState(localNumberForCountry(applicant.contactNumber, initialCountry));
