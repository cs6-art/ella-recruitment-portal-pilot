...
type CalendarLookupState = "loading" | "ready" | "error";
...
function bookingFromSavedException(input: Pick<InterviewBooking, "roleId" | "date" | "startTime" | "endTime" | "timezone">, responseSlot: unknown): InterviewBooking {
  const responseRecord = responseSlot && typeof responseSlot === "object" ? responseSlot as Record<string, unknown> : {};
  // Sheets returns the booking directly; the Postgres target wraps the row in
  // { slot, created, error }. The calendar only needs the stable id and the
  // values already validated by the form, so support both response shapes.
  const savedSlot = responseRecord.slot && typeof responseRecord.slot === "object" ? responseRecord.slot as Record<string, unknown> : responseRecord;
  return {
    slotId: String(savedSlot.slotId ?? savedSlot.id ?? slotKey({ interviewType: "Final Interview", ...input })),
    ...
    calendarEventId: String(savedSlot.calendarEventId ?? ""),
    calendarEventLink: String(savedSlot.calendarEventLink ?? ""),
    calendarEventStatus: String(savedSlot.calendarEventStatus ?? ""),
    calendarEventError: String(savedSlot.calendarEventError ?? ""),
  };
}
  ...
  useEffect(() => {
    ...
    void fetch("/api/bookings/calendar-busy", { credentials: "same-origin", cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        ...
        setCalendarLookupState("ready");
      })
      .catch((caught: unknown) => {
        if (caught instanceof DOMException && caught.name === "AbortError") return;
        console.warn("[Bookings] Calendar conflict lookup failed:", caught);
        setCalendarLookupState("error");
      });
    return () => controller.abort();
  }, []);
  const roleOptions = useMemo(() => roles
    .filter((role) => role.roleId.trim())
    .map((role) => ({ ...role, finalBusyWindows: JSON.stringify(calendarBusyWindows[role.roleId] || []), finalCalendarConnected: calendarConnected[role.roleId] }))
