...
export const GET = withInternalAuth("applications", async (request) => {
  ...
  const items = await listApplications(params.get("stage") || undefined, params.get("roleExternalId") || undefined, request.headers.get("x-organization-id")?.trim() || undefined);
...
export const POST = withInternalAuth("applications", async (request) => {
  ...
  const result = await createApplication({ externalId: String(body.externalId), applicantEmail: String(body.applicantEmail), roleExternalId: String(body.roleExternalId), organizationId: request.headers.get("x-organization-id")?.trim() || undefined, applicantName: typeof body.applicantName === "string" ? body.applicantName : undefined, phone: typeof body.phone === "string" ? body.phone : undefined, preferredMobile: typeof body.preferredMobile === "string" ? body.preferredMobile : undefined, applicantCountry: typeof body.applicantCountry === "string" ? body.applicantCountry : undefined, source: typeof body.source === "string" ? body.source : undefined, sourceDetail: typeof body.sourceDetail === "string" ? body.sourceDetail : undefined, consentAt: typeof body.consentAt === "string" ? body.consentAt : undefined });
