...
export const POST = withInternalAuth("booking", async (request) => {
  ...
  const result = await createInterviewSlot({ interviewType: body.interviewType, slotCode: typeof body.slotCode === "string" ? body.slotCode : undefined, roleExternalId: typeof body.roleExternalId === "string" ? body.roleExternalId : undefined, organizationId: request.headers.get("x-organization-id")?.trim() || undefined, startsAt: String(body.startsAt), endsAt: String(body.endsAt), timezone: String(body.timezone) });
