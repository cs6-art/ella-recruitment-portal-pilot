...
import { PRE_INTERVIEW_STATUSES } from "@/lib/live-interview";
import { getCandidateInterviewStatus } from "@/lib/live-interview-store";
...
export default async function AvatarInterviewPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!context) {
    // A reopened or refreshed link after the interview started: show its
    // status (and save an interrupted interview) instead of a dead end.
    const interview = isPostgresRecruitmentTarget() ? await getCandidateInterviewStatus(token).catch(() => null) : null;
    if (!interview || PRE_INTERVIEW_STATUSES.includes(interview.status)) notFound();
    return (
  ...
  if (!isLiveAvatarConfigured()) notFound();
