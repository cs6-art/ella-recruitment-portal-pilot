...
export function buildScreeningPrompt(input: {
  ...
  return [
    ...
    "Do not award a high score for generic confidence, unrelated design experience, or a list of tools. Require direct evidence of the role's required skills and responsibilities in the resume.",
    "If a required domain skill is absent, contradicted, or only mentioned without hands-on evidence, reflect that clearly in gaps and reduce the score materially. A candidate with no evidence of the core role should not receive the same recommendation strength as a candidate who demonstrates it.",
    "Flag implausible or internally inconsistent experience claims for HR verification; do not silently accept them as evidence.",
